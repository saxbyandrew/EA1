...
void EAStrategyParameters::resetValues() {
      ...
      sb.strategyNumber=0;            // Name of strategy for comment 
      sb.magicNumber=0;
      sb.deviationInPoints=0; 
      sb.maxSpread=0;                 // Limit the spread to non stupid values
      //sb.runOnFrequency=0; 
      sb.marketOpenDelay=0;           // In minutes                   
      sb.marketCloseDelay=0;          // In minutes  minus -30 to close 30 before end of session  
      sb.marketSessions[_FIRST_SESSION]=_NO;   // _YES _NO of session to use     
      sb.marketSessions[_SECOND_SESSION]=_NO;       
      sb.marketSessions[_THIRD_SESSION]=_NO;
      ...
      sb.lotSize=0; 
      sb.fptl=0;      // Dollar Value
      sb.fltl=0;       // same
      sb.fpts=0;     // same
      sb.flts=0;       // Dollar Valu
      ...
      sb.dnnShortNumber=0;
      sb.maxMg=0;          
      sb.maxLongHedgeLoss=0;
      sb.multiMg=0;  
      sb.maxLong=0;
      sb.maxShort=0;
      sb.maxDaily=0;
      sb.maxDailyHold=0;
      sb.optimizationLong=0;
      sb.optimizationShort=0;
      sb.optimizationMartingale=0;
      sb.optimizationHedge=0;
      sb.swapCosts=0;
...
void EAStrategyParameters::loadSQLStrategy() {
   ...
   DatabaseRead(request);
      ...
      DatabaseColumnInteger   (request,4,sb.runMode);
      DatabaseColumnInteger   (request,5,sb.defaultRunMode);
      DatabaseColumnInteger   (request,6,sb.magicNumber);
      DatabaseColumnInteger   (request,7,sb.deviationInPoints);
      DatabaseColumnInteger   (request,8,sb.maxSpread);
      DatabaseColumnInteger   (request,9,sb.entryBars);
      DatabaseColumnDouble    (request,10,sb.brokerAdminPercent);
      DatabaseColumnDouble    (request,11,sb.interBankPercentage);
      DatabaseColumnInteger   (request,12,sb.sessionTradingTime);
      DatabaseColumnText      (request,13,sb.tradingStart);
      DatabaseColumnText      (request,14,sb.tradingEnd);
      DatabaseColumnText      (request,15,sb.marketSessions1);
      DatabaseColumnText      (request,16,sb.marketSessions2);
      DatabaseColumnText      (request,17,sb.marketSessions3);
      DatabaseColumnInteger   (request,18,sb.marketOpenDelay);
      DatabaseColumnInteger   (request,19,sb.marketCloseDelay);
      DatabaseColumnInteger   (request,20,sb.inProfitClosePosition);
      DatabaseColumnInteger   (request,21,sb.inLossClosePosition);
      DatabaseColumnInteger   (request,22,sb.inLossOpenMartingale);
      ...
      DatabaseColumnInteger   (request,30,sb.optimizationLong);
      DatabaseColumnInteger   (request,31,sb.optimizationShort);
      DatabaseColumnInteger   (request,32,sb.optimizationMartingale);
      DatabaseColumnInteger   (request,33,sb.optimizationHedge);
      DatabaseColumnDouble    (request,34,sb.swapCosts);
      DatabaseColumnInteger   (request,35,sb.dnnType);
      DatabaseColumnInteger   (request,36,sb.dnnIn);
      DatabaseColumnInteger   (request,37,sb.dnnLayer1);
      DatabaseColumnInteger   (request,38,sb.dnnLayer2);
      DatabaseColumnInteger   (request,39,sb.dnnOut);
      DatabaseColumnInteger   (request,40,sb.dataFrameSize);
      DatabaseColumnInteger   (request,41,sb.dataFrameStartBar);
      DatabaseColumnDouble    (request,42,sb.lotSize);
      DatabaseColumnDouble    (request,43,sb.fptl);
      DatabaseColumnDouble    (request,44,sb.fltl);
      DatabaseColumnDouble    (request,45,sb.fpts);
      DatabaseColumnDouble    (request,46,sb.flts);
      DatabaseColumnInteger   (request,47,sb.maxLong);
      DatabaseColumnInteger   (request,48,sb.maxShort);
      DatabaseColumnInteger   (request,49,sb.maxDaily);
      DatabaseColumnInteger   (request,50,sb.maxDailyHold);
      DatabaseColumnInteger   (request,51,sb.maxMg);
      DatabaseColumnInteger   (request,52,sb.multiMg);
      DatabaseColumnDouble    (request,53,sb.maxLongHedgeLoss);
   ...
   if (sb.marketSessions1=="YES") {
      sb.marketSessions[0]=_YES;
   } else {
      sb.marketSessions[0]=_NO; 
   }     
   if (sb.marketSessions2=="YES") {
      sb.marketSessions[1]=_YES; 
   } else {
      sb.marketSessions[1]=_NO; 
   } 
   if (sb.marketSessions3=="YES") {
      sb.marketSessions[2]=_YES;
   } else {
   ...
   switch (sb.dnnType) {
      ...
      break;
      case 2:  sb.dnnType=_SHORT;
      break;
      case 4:  sb.dnnType=_HEDGE;
      break;
