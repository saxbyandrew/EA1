...
void EAPanel::createPanel(string name,int subWindow,int x1,int y1,int x2,int y2) {
      ...
      //refreshButton=new CButton;
      //refreshButton.Create(0,"refreshButton",0,20,20, 170, 45);
      //refreshButton.Corner(CORNER_RIGHT_LOWER);
...
void EAPanel::mainInfoPanel() {
   ...
   updateInfo1Value(0,IntegerToString(param.strategyNumber));

   updateInfo1Label(1, "Trading Time");  
   

   if (param.sessionTradingTime==_ANYTIME) {
      updateInfo1Value(1,"Any Time");
   }

   if (param.sessionTradingTime==_FIXED_TIME) {
      updateInfo1Value(1,"Fixed Times");

      updateInfo1Label(2, "Trading Start");  
      updateInfo1Value(2,param.tradingStart);
      updateInfo1Label(3, "Trading End");  
      updateInfo1Value(3,param.tradingEnd);
   }

   if (param.sessionTradingTime==_SESSION_TIME) {
      ...
      if (param.marketOpenDelay!=0) {
         updateInfo1Label(4, "Market Open Delay");  
         updateInfo1Value(4,IntegerToString(param.marketOpenDelay));
      } else {
         updateInfo1Label(4, "Market Open Delay");  
         updateInfo1Value(4, "No Delay");
      }

      if (param.marketCloseDelay!=0) {
         updateInfo1Label(5, "Market Close Delay");  
         updateInfo1Value(5,IntegerToString(param.marketCloseDelay));
      } else {
         updateInfo1Label(5, "Market Close Delay");  
   ...
   if (bool (param.closingTypes&_CLOSE_AT_EOD)) {
   ...
   //if (param.maxTotalDailyPositions==-1) {
      //updateInfo1Value(9,"No Maximum");
   //} else {
      //updateInfo1Value(9,StringToInteger(param.maxTotalDailyPositions));
   //}

   updateInfo1Label(10, "Max day to hold"); 
   if (param.maxTotalDaysToHold>0) {
      updateInfo1Value(10,IntegerToString(param.maxTotalDaysToHold));
   } else {
   ...
   if (bool (param.closingTypes&_IN_PROFIT_CLOSE_POSITION)) {
         updateInfo2Value(0,"Yes");
         updateInfo2Label(2, "Profit Long");  
         updateInfo2Value(2,StringFormat("$%5.2f",param.fixedProfitTargetLong));
         updateInfo2Label(4, "Profit Short");  
         updateInfo2Value(4,StringFormat("$%5.2f",param.fixedProfitTargetShort));
   ...
   if (bool (param.closingTypes&_IN_LOSS_CLOSE_POSITION)) {
         updateInfo2Value(1,"Yes");
         updateInfo2Label(3, "Loss Long");  
         updateInfo2Value(3,StringFormat("$%5.2f",param.fixedLossTargetLong));
         updateInfo2Label(5, "Loss Short");  
         updateInfo2Value(5,StringFormat("$%5.2f",param.fixedLossTargetShort));
   } else {
   ...
   if (param.inLossOpenLongHedge) {
         updateInfo2Value(6,"Yes");
         updateInfo2Label(7, "Hedge Loss Amt");  
         updateInfo2Value(7,StringFormat("$%5.2f",param.maxLongHedgeLossAmountAllowed));
         updateInfo2Label(8, "Hedge Number"); 
         updateInfo2Value(8,StringFormat("%d",param.dnnHedgeNumber));
   } else {
   ...
   if (param.inLossOpenMartingale) {
         updateInfo2Value(9,"Yes");
         updateInfo2Label(10, "Martingale Positions");  
         updateInfo2Value(10,IntegerToString(param.maxMartingalePositions));
         updateInfo2Label(11, "Martingale multiplier");  
         updateInfo2Value(11,IntegerToString(param.martingaleMultiplier));
         updateInfo2Label(12, "Martingale Number"); 
         updateInfo2Value(12,StringFormat("%d",param.dnnMartingaleNumber));
   ...
   updateInfo2Value(15,DoubleToString(param.lotSize));

   if (param.maxPositionsLong>0) {
      updateInfo2Label(16, "Max allowed Long"); 
      updateInfo2Value(16,IntegerToString(param.maxPositionsLong));
   } else {
      updateInfo2Label(16, "No Long position allowed"); 
      updateInfo2Value(16,"-");
   }
   
   if (param.maxPositionsShort>0) {
   updateInfo2Label(17, "Max allowed Short"); 
   updateInfo2Value(17,IntegerToString(param.maxPositionsShort));
   } else {
...
void EAPanel::accountInfoUpdate() {
      ...
      updateInfo1Label(23,StringFormat("Total Swap Costs: $%3.2f",usingStrategyValue.swapCosts)); 
      updateInfo1Value(23,"");
